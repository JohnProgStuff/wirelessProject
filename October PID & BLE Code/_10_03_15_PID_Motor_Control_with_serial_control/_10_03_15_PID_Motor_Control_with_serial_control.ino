    ...
    if (inputString == ShiftUP) { // Button 1 pressed 1st sample  
        ...
        if (gear != numCogs || tuneON == 1){
           ...
           while(encoderCounter <= SetTarget){
              ...
              //while((-3) < (encoderCounter - encCount) < 3){ // need to make this an absolute value
                myPID.Compute();
...
//              driveArdumoto(MOTOR_B, CW, PID.); // Set motor B to CCW at max                      
            }//end while  
            stopArdumoto(MOTOR_B); 
...
//   String Bedge = RISING; // start with RISING
void enc_B_ISR(void){
      int tc = 0;
...
void SerialFromPC(void){
  ...
  if (Serial.available()) { // if Serial buffer has stuff
      ...
      //Serial.print("5"); // echo each char back to PC
