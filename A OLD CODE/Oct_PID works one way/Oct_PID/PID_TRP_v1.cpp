...
bool PID::Compute()
{
   ...
   if(timeChange>=SampleTime)
   {
	  ...
	  */
	  ...
	  int input = *myInput;
      int error = *mySetpoint - input;   // 285-295 = -10
	  if (error < 0){ error *= (-1); Serial.println("OS");}
	  Serial.print("\nerr: ");	  Serial.print(error);
	  Serial.print(" , ki: ");	  Serial.print(ki*10000);
     
	 ITerm += (ki * error);
      //ITerm += (error);
      //Serial.print("err: ");  Serial.print(error);
	  Serial.print(", I: ");  Serial.println(ITerm);
...
void PID::SetTunings(int Kp, int Ki, int Kd)
{
   ...
   kp = Kp;
   ki = Ki * SampleTimeInSec; // ki will always be zero if it is a int
   kd = Kd / SampleTimeInSec; // kd will always be zero if it is a int
